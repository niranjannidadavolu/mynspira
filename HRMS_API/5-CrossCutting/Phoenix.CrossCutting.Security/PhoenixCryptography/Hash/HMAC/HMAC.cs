...
using Phoenix.CrossCutting.Security.PhoenixCryptography.Hash.HMAC.Enum;
using Phoenix.CrossCutting.Security.PhoenixCryptography.Util;

namespace Phoenix.CrossCutting.Security.PhoenixCryptography.Hash.HMAC
{
    /// <summary>
        ...
        public static HashResult Encrypt(HMACAlgorithmTypes type, string data, string key, Encoding encoding = null)
        {
            switch (type)
            {
                case HMACAlgorithmTypes.MD5:
                    return EncryptResultInternal<HMACMD5>(data, key, encoding);
#if NFX
                ...
                case HMACAlgorithmTypes.SHA1:
                    return EncryptResultInternal<HMACSHA1>(data, key, encoding);
                case HMACAlgorithmTypes.SHA256:
                    return EncryptResultInternal<HMACSHA256>(data, key, encoding);
                case HMACAlgorithmTypes.SHA384:
                    return EncryptResultInternal<HMACSHA384>(data, key, encoding);
                case HMACAlgorithmTypes.SHA512:
                    return EncryptResultInternal<HMACSHA512>(data, key, encoding);
                default:
