                ...
                if (ch == '<')
                {
                    ...
                    while (pos < path.Length)
                    {
                        ...
                        if (ch == '\'')
                        {
                            ...
                            pos = Text.StringBuilder.Extensions.GetIndexAfterNextSingleQuote(path, pos, true);
                        }
                        else if (ch == '"')
                        {
                            // SKIP attribute starting with double quote
                            pos = Text.StringBuilder.Extensions.GetIndexAfterNextDoubleQuote(path, pos, true);
                        }
                    }
                }
                else
